...
function renderSingle(book) {
    ...
    author.classList = 'lead';
    fetch(book.author, {method: 'HEAD'})
        .then(res => {
            if (res.ok) {
                author.textContent = book.author;
            } else {
                author.textContent = 'Autors nav atrasts';
            }
        })
    author.textContent = book.author;
    ...
    if (book.category.length > 0) {
        let categoryLabel = document.createElement('dt');
        categoryLabel.classList = 'col-sm-3';
        categoryLabel.textContent = 'Žanrs';
        dl.appendChild(categoryLabel);

        let categoryValue = document.createElement('dd');
        categoryValue.classList = 'col-sm-9';
        categoryValue.textContent = book.category;
        dl.appendChild(categoryValue);
    }
...
function renderRelated(books) {
    ...
    for (const book of books) {
        ...
        // create card image
        let img = document.createElement('img');
        img.classList = 'card-img-top books-img';
        fetch(book.image, {method: 'HEAD'})
            .then(res => {
                if (res.ok) {
                    img.src = book.image;
