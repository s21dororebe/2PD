@extends('layout') @section('content')
    ...
    <form method="post" action="{{ $book->exists ? '/books/patch/' . $book->id : '/books/put' }}"
        ...
        <div class="mb-3"><label for="book-name" class="form-label">Nosaukums</label> <input type="text" id="book-name"
                                                                                             name="name"
        ...
        <div class="mb-3"><label for="book-price" class="form-label">Cena</label> <input type="number" min="0.00"
                                                                                         ...
                                                                                         class="form-control @error('price') is-invalid @enderror"> @error('price')
            <p class="invalid-feedback">{{ $errors->first('price') }}</p> @enderror </div>
        <div class="mb-3"><label for="book-image" class="form-label">Attēls</label> @if ($book->image)
                <img src="{{ asset('images/' . $book->image) }}" class="img-fluid img-thumbnail d-block mb-2"
                     alt="{{ $book->name }}">
            @endif <input type="file" accept="image/png, image/jpeg" id="book-image" name="image"
                          class="form-control @error('image') is-invalid @enderror"> @error('image') <p
                class="invalid-feedback">{{ $errors->first('image') }}</p> @enderror </div>
        <div class="mb-3">
            <div class="form-check"><input type="checkbox" id="book-display" name="display" value="1"
                                           class="form-check-input @error('display') is-invalid @enderror"
                                           @if (old('display', $book->display)) checked @endif > <label
                    class="form-check-label" for="book-display"> Publicēt ierakstu </label>
                @error('display') <p class="invalid-feedback">{{ $errors->first('display') }}</p> @enderror </div>
        </div>
        <button type="submit"
                class="btn btn-primary"> {{ $book->exists ? 'Atjaunot ierakstu' : 'Pievienot ierakstu' }} </button>
    </form>
@endsection
